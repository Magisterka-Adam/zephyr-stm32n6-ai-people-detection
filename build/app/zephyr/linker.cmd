...
MEMORY
    {
    FLASH (rx) : ORIGIN = 0x34000000, LENGTH = (0 * 1024 - 0x0 - 0x180)
    RAM (wx) : ORIGIN = 0x34000000, LENGTH = (2048 * 1K)
    AXISRAM3 ( rw ) : ORIGIN = (874512384), LENGTH = (458752) AXISRAM4 ( rw ) : ORIGIN = (874971136), LENGTH = (458752) AXISRAM5 ( rw ) : ORIGIN = (875429888), LENGTH = (458752) AXISRAM6 ( rw ) : ORIGIN = (875888640), LENGTH = (458752) AXISRAM1 ( rw ) : ORIGIN = (872415232), LENGTH = (2097152) AXISRAM2 ( rw ) : ORIGIN = (873989120), LENGTH = (523264) PSRAM ( rw ) : ORIGIN = (2415919104), LENGTH = (33554400)
...
ztest :
{
 ...
 input_callback_area : { _input_callback_list_start = .; KEEP(*(SORT_BY_NAME(._input_callback.static.*))); _input_callback_list_end = .;; } > RAM
 symbol_to_keep :
 {
...
video_device_area : { _video_device_list_start = .; KEEP(*(SORT_BY_NAME(._video_device.static.*))); _video_device_list_end = .;; } > RAM
    __data_region_end = .;
ASSERT((__kernel_ram_start < (873989120 - 0x400)),
       "
...
/DISCARD/ :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    AXISRAM3 (NOLOAD) : { __AXISRAM3_start = .; KEEP(*(AXISRAM3)) KEEP(*(AXISRAM3.*)) __AXISRAM3_end = .; } > AXISRAM3 __AXISRAM3_size = __AXISRAM3_end - __AXISRAM3_start; __AXISRAM3_load_start = LOADADDR(AXISRAM3); AXISRAM4 (NOLOAD) : { __AXISRAM4_start = .; KEEP(*(AXISRAM4)) KEEP(*(AXISRAM4.*)) __AXISRAM4_end = .; } > AXISRAM4 __AXISRAM4_size = __AXISRAM4_end - __AXISRAM4_start; __AXISRAM4_load_start = LOADADDR(AXISRAM4); AXISRAM5 (NOLOAD) : { __AXISRAM5_start = .; KEEP(*(AXISRAM5)) KEEP(*(AXISRAM5.*)) __AXISRAM5_end = .; } > AXISRAM5 __AXISRAM5_size = __AXISRAM5_end - __AXISRAM5_start; __AXISRAM5_load_start = LOADADDR(AXISRAM5); AXISRAM6 (NOLOAD) : { __AXISRAM6_start = .; KEEP(*(AXISRAM6)) KEEP(*(AXISRAM6.*)) __AXISRAM6_end = .; } > AXISRAM6 __AXISRAM6_size = __AXISRAM6_end - __AXISRAM6_start; __AXISRAM6_load_start = LOADADDR(AXISRAM6); AXISRAM1 (NOLOAD) : { __AXISRAM1_start = .; KEEP(*(AXISRAM1)) KEEP(*(AXISRAM1.*)) __AXISRAM1_end = .; } > AXISRAM1 __AXISRAM1_size = __AXISRAM1_end - __AXISRAM1_start; __AXISRAM1_load_start = LOADADDR(AXISRAM1); AXISRAM2 (NOLOAD) : { __AXISRAM2_start = .; KEEP(*(AXISRAM2)) KEEP(*(AXISRAM2.*)) __AXISRAM2_end = .; } > AXISRAM2 __AXISRAM2_size = __AXISRAM2_end - __AXISRAM2_start; __AXISRAM2_load_start = LOADADDR(AXISRAM2); PSRAM (NOLOAD) : { __PSRAM_start = .; KEEP(*(PSRAM)) KEEP(*(PSRAM.*)) __PSRAM_end = .; } > PSRAM __PSRAM_size = __PSRAM_end - __PSRAM_start; __PSRAM_load_start = LOADADDR(PSRAM);
.last_section :
{
  KEEP(*(.last_section))
} > RAM
...
noinit (NOLOAD) :
{
    ...
    __kernel_ram_end = 0x34000000 + (2048 * 1K);
